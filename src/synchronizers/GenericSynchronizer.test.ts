...
describe(GenericSynchronizer.name, () => {
  ...
  it("syncs past correctly", async () => {
    ...
    expect(
      await synchronizerStatusRepository.findByName(
        synchronizerName,
        domainName
      )
    ).toEqual(expect.objectWith({ block: currentBlock + 1 }));
  });
  ...
  it("syncs tip", async () => {
    ...
    ).toEqual(expect.objectWith({ block: 5 }));
  ...
  it("restarts after being done from the last block", async () => {
    ...
    await waitForExpect(async () => {
      ...
      ).toEqual(expect.objectWith({ block: 6 }));
    });
  });
