...
export abstract class GenericSynchronizer {
  ...
  async run(): Promise<void> {
    this.setSyncing();
    const syncStatus = await this.synchronizerStatusRepository.findByName(
      this.syncName,
      this.domainName
    );
    ...
    while (this.state !== "stopped") {
      const currentBlock =
        (await this.blockchain.getLatestBlockNumber()) -
        this.options.safeDistanceFromTip;
      const toBlockNumber = Math.min(
        fromBlockNumber + this.blocksPerBatch,
        currentBlock + 1
      ); // exclusive
  ...
  abstract sync(
    from: number,
    to: number
  ): Promise<((tx: TxHandle) => Promise<any>) | void>;
}
