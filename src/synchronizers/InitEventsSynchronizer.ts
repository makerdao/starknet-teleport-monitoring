...
export class InitEventsSynchronizer extends GenericSynchronizer {
  ...
  async sync(from: number, to: number) {
    ...
    console.log(
      `[${this.syncName}] Found ${newTeleports.length} new teleports`
    );

    const modelsToCreate: Omit<Teleport, "id">[] = newTeleports.map(
      (w: any) => {
    ...
    return (tx: TxHandle) =>
      this.teleportRepository.createMany(modelsToCreate, tx);
  }
}
