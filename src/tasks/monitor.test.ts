...
describe("Monitoring", () => {
  ...
  it("detects bad debt", async () => {
    ...
    await sdk.oracleAuth
      .connect(receiver)
      .requestMint(teleport, signatures, 0, 0);
    console.log(
      `Printing unbacked DAI done at block ${await hhProvider.getBlockNumber()}`
    );
    ...
    await waitForExpect(() => {
      expect(
        metrics[
          'teleport_bad_debt{domain="STARKNET-MASTER-1",network="goerli"}'
        ]
      ).toEqual(daiToMint.toString());
    }, 10_000);
  });
