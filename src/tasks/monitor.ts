...
import {
  getL1SdkBasedOnNetworkName,
  getL2SdkBasedOnNetworkName,
} from "../sdks";
import {
...
export async function monitor({
  ...
  for (const slave of networkConfig.slaves) {
    const l2Provider = new Provider(slave.l2Rpc);
  ...
  // monitor
  const { cancel: cancelBlockWatcher } = await onEveryFinalizedBlock(
    async (blockNumber) => {
      ...
      if (allSynced) {
        ...
        const previousBadDebt = BigNumber.from(
          metrics[makeMetricName("teleport_bad_debt", labels)] || 0
        );

        metrics[makeMetricName("teleport_bad_debt", labels)] = previousBadDebt
          .add(newBadDebt)
          .toString();
      }
      ...
      for (const slave of networkConfig.slaves) {
        ...
        metrics[makeMetricName("teleport_l1_dai_balance", labels)] =
          balances.l1Balance;
        metrics[makeMetricName("teleport_l2_dai_balance", labels)] =
          balances.l2Balance;
        metrics[makeMetricName("teleport_bad_debt_l1_block", labels)] =
          blockNumber;
        ...
        if (allSynced) {
          ...
          metrics[makeMetricName("teleport_last_flush_ms", labels)] =
            sinceLastFlush;
          metrics[makeMetricName("teleport_debt_to_flush", labels)] =
            debtToFlush;
          ...
          metrics[makeMetricName("teleport_last_settle_ms", labels)] =
            sinceLastSettle;
          metrics[makeMetricName("teleport_debt_to_settle", labels)] =
            debtToSettle;
        }
      }
    },
