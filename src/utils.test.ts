...
describe(makeMetricName.name, () => {
  ...
  it("makes a metric name with 1 label", () => {
    expect(makeMetricName("cpu_usage", { location: "FRA" })).toEqual(
      'cpu_usage{location="FRA"}'
    );
  });

  it("makes a metric name with multiple labels", () => {
    expect(
      makeMetricName("cpu_usage", { location: "FRA", arch: "amd64" })
    ).toEqual('cpu_usage{location="FRA",arch="amd64"}');
  });
});
