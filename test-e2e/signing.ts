...
export async function getAttestations(
  ...
  const teleportEncoded: TeleportGUID = {
    ...
    amount: ethers.utils.hexZeroPad(
      BigNumber.from(teleportGUID.amount).toHexString(),
      32
    ),
    nonce: ethers.utils.hexZeroPad(
      BigNumber.from(teleportGUID.nonce).toHexString(),
      32
    ),
    timestamp: ethers.utils.hexZeroPad(
      BigNumber.from(teleportGUID.timestamp).toHexString(),
      32
    ),
  };
...
async function signTeleportGUID(
  ...
  const sigs = await Promise.all(
    signers.map((signer) => signer.signMessage(arrayify(guidHash)))
  );
